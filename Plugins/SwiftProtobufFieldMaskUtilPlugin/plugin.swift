...
@main
struct SwiftProtobufFieldMaskUtilPlugin {
    /// Errors thrown by the `SwiftProtobufFieldMaskUtilPlugin`
    ...
    func createBuildCommands(
        ...
        let protocGenFieldMaskUtilSwiftPath = try tool("protoc-gen-fieldmaskutil-swift").path
        ...
        return configuration.invocations.map { invocation in
            self.invokeProtoc(
                ...
                protocGenFieldMaskUtilSwiftPath: protocGenFieldMaskUtilSwiftPath,
    ...
    ///   - protocGenFieldMaskUtilSwiftPath: The path to the `protoc-gen-fieldmaskutil-swift`
    ///   binary.
    ...
    private func invokeProtoc(
        ...
        protocPath: Path,
        protocGenFieldMaskUtilSwiftPath: Path,
        outputDirectory: Path
    ) -> Command {
        // Construct the `protoc` arguments.
        var protocArgs = [
            "--plugin=protoc-gen-fieldmaskutil-swift=\(protocGenFieldMaskUtilSwiftPath)",
            "--fieldmaskutil-swift_out=\(outputDirectory)",
        ]
        ...
        if let visibility = invocation.visibility {
            protocArgs.append("--fieldmaskutil-swift_opt=Visibility=\(visibility.rawValue)")
        }

        // Add the file naming if it was set
        if let fileNaming = invocation.fileNaming {
            protocArgs.append("--fieldmaskutil-swift_opt=FileNaming=\(fileNaming.rawValue)")
        }
        ...
        if let implementationOnlyImports = invocation.implementationOnlyImports {
            protocArgs.append("--fieldmaskutil-swift_opt=ImplementationOnlyImports=\(implementationOnlyImports)")
        }
        ...
        for var file in invocation.protoFiles {
            ...
            file.removeLast(5)
            file.append("fieldmaskutil.swift")
        ...
        return Command.buildCommand(
            displayName: "Generating swift FieldMaskUtil files from proto files",
            executable: protocPath,
            arguments: protocArgs,
            inputFiles: inputFiles + [protocGenFieldMaskUtilSwiftPath],
...
extension SwiftProtobufFieldMaskUtilPlugin: BuildToolPlugin {
    func createBuildCommands(
...
extension SwiftProtobufFieldMaskUtilPlugin: XcodeBuildToolPlugin {
    func createBuildCommands(
